...
parse_genloc_gprmc_test_() ->
	[
		...
		{"Ensure we correctly parse GPS receiver status",
		fun() ->
			Body = <<"        $GPRMC,163734.00,A,4434.34454,N,00046.44022,E,0.015,0.00,230917,,,A*67\n"
			"        $GPRMC,170818.00,V,,,,,,,170817,,,N*72">>,
			User = #{
				device => <<"ercogener_genloc_341e">>,
				dumps_incoming => true,
				email => <<"test@ytotech.com">>,
			...
			ParsedExpected = [
				...
				{ok,#{
					...
					latitude => nil,
					longitude => nil,
					raw => <<"$GPRMC,170818.00,V,,,,,,,170817,,,N*72">>,
					receiver_status_code => <<"V">>,
					receiver_status => warning,
					time => {17,8,18.0},
					timestamp => {{2017,8,17},{17,8,18.0}}
				}}
			],
...
parse_genloc_gploc_test_() ->
	[
		...
		{"Ensure we correctly parse GPS receiver status",
		fun() ->
			Body = <<"        $GPLOC,358683066123549,A,1,081247.00,4834.36782,N,00105.95857,W,01100,3194857900003819*2D\n"
			% TODO Put a real(ist) payload.
			"        $GPLOC,358683066123549,V,0,081247.00,,,,,01100,3194857900003819*2D $GPRMC,170818.00,V,,,,,,,170817,,,N*72">>,
			User = #{
				device => <<"ercogener_genloc_341e">>,
				dumps_incoming => true,
				email => <<"test@ytotech.com">>,
			...
			ParsedExpected = [
				...
				{ok,#{
					...
					latitude => nil,
					longitude => nil,
					raw => <<"$GPLOC,358683066123549,V,0,081247.00,,,,,01100,3194857900003819*2D $GPRMC,170818.00,V,,,,,,,170817,,,N*72">>,
					receiver_status_code => <<"V">>,
					receiver_status => warning,
					time => {8,12,47.0},
					timestamp => nil
